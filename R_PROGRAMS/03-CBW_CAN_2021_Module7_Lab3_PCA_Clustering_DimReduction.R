...
### 1) Loads integrated datasets R object produced by script `CBW_CAN_SingleCell_2_Integration.R'
...
### 'Rscript ~/path_to/CBW_CAN_SingleCell_3_PCA_Clustering_DimReduction.R'
####################################
...
PathToIntegratedRds <- paste0("~/CourseData/CAN_data/Module7/OUTFILES/Richards_NatCancer_2021/", AnchorFinder, "/WITHOUT_G945_I_T", "/Richards_NatCancer_2021_Integration.rds") ### /path_to/*rds (R object) from CBW_CAN_SingleCell_2_Integration.R
...
SampleForInferCNV   <- "G983_A_T" ## Filtered counts and cell cluster identities from this sample will be used for CBW_CAN_SingleCell_6_InferCNV.R
...
################################################################################################################################################
################################################################################################################################################
### HERE ARE THE FUNCTIONS TO REDUCE DIMENSIONS
################################################################################################################################################
################################################################################################################################################
...
################################################################################################################################################
################################################################################################################################################
### HERE ARE THE FUNCTIONS TO CLUSTER CELLS
################################################################################################################################################
################################################################################################################################################

writeLines("\n**** CLUSTER INTEGRATED DATASETS ****\n")
...
writeLines("\n*** Print out the SCT integrated matrix and cell cluster identities for CBW_CAN_SingleCell_6_InferCNV.R ***\n")
...
################################################################################################################################################
################################################################################################################################################
### HERE ARE THE FUNCTIONS TO COLOUR UMAP PLOTS USING CELL PROPERTIES
################################################################################################################################################
################################################################################################################################################
...
####################################
writeLines(paste0("\n*** Colour ", DimensionReductionMethods[[DimRedMethodPlots]][["name"]], " plot by global cell-clusters ***\n"))
...
####################################
writeLines(paste0("\n*** Colour ", DimensionReductionMethods[[DimRedMethodPlots]][["name"]], " plot by dataset ***\n"))

plots <- DimPlot(seurat.object.integrated, group.by = c("dataset"), combine = F, reduction = DimRedMethodPlots)
plots <- lapply(X = plots, FUN = function(x) x + theme(legend.position = "top") + guides(color = guide_legend(override.aes = list(size = 3))))
...
writeLines(paste0("\n*** Colour ", DimensionReductionMethods[[DimRedMethodPlots]][["name"]], " for all cells by selected genes ***\n"))
...
writeLines(paste0("\n*** Colour ", DimensionReductionMethods[[DimRedMethodPlots]][["name"]], " using cell-level metadata ***\n"))
...
sapply(MetedataPropsToPlot, FUN=function(property) {
...
writeLines(paste0("\n*** Colour ", DimensionReductionMethods[[DimRedMethodPlots]][["name"]], " plot by global cell-clusters, subsetting each dataset ***\n"))

sapply(unique(seurat.object.integrated@meta.data$dataset), FUN=function(datasetID) {
  Idents(seurat.object.integrated) <- "dataset"
...
################################################################################################################################################
################################################################################################################################################
### HERE ARE THE FUNCTIONS TO GET OTHER PLOT TYPES
################################################################################################################################################
################################################################################################################################################
...
plots <- DotPlot(seurat.object.integrated, features = SelectedGenes, idents = ClustersForDotPlot2,
                 cols = c("cornflowerblue", "darkolivegreen4", "chocolate1", "aquamarine3", "burlywood3", 
                          "azure4", "brown1"), dot.scale = 8, split.by = "dataset")
plots <- plots + RotatedAxis()
...
################################################################################################################################################
################################################################################################################################################
### HERE ARE THE FUNCTIONS TO GET AVERAGE GENE EXPRESSION
################################################################################################################################################
################################################################################################################################################
...
writeLines("\n*** Saving the PCA_Clustering_DimReduction R object ***\n")
